...
public class ChildAgent : Agent
{
    ...
    public override void Initialize()
    {
        ...
        Parent = rewardSpec.parentRef.gameObject;
    }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        Vector3 Parentsize = Parent.GetComponent<Collider>().bounds.size;
        ...
        Vector3 toParent = (Parent.transform.position - transform.position).normalized;
        ...
        // ✅ 부모 Freeze 여부
        var parentAgent = Parent.GetComponent<ParentAgent>();
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        float ParentDist = float.MaxValue;

        Vector3 p1 = myCol.ClosestPoint(Parent.transform.position);
        Vector3 p2 = Parent.GetComponent<Collider>().ClosestPoint(p1);
        p1.y = 0; p2.y = 0; // ✅ Y 무시
        ParentDist = Vector3.Distance(p1, p2);

        // ✅ 거리 shaping 보상 (이전보다 가까워졌으면 +, 멀어지면 -)
        if (prevTargetDist < float.MaxValue) // 첫 step은 비교 불가
        {
            float delta = prevTargetDist - ParentDist;
            reward += delta * rewardSpec.wDist * 100f;
            // Debug.Log(delta * rewardSpec.wDist * 100f);
        }
        prevTargetDist = ParentDist; // 다음 step을 위해 업데이트
        ...
        targetDisOK = ParentDist < (rewardSpec.targetDistance + rewardSpec.freezeTolerance);
        targetRotOK = cosVal > rewardSpec.angleThresholdCos;

        bool canFreeze = true;
        // 부모에도 FurnitureAgent 스크립트가 붙어있어야 함
        var parentAgent = rewardSpec.parentRef.GetComponent<ParentAgent>();
        if (parentAgent != null && !parentAgent.isFrozen)
        {
            // 부모가 아직 고정되지 않음 → 자식도 고정하면 안 됨
            canFreeze = false;
        }
        if (targetDisOK && targetRotOK && canFreeze)
        {
            isFrozen = true;
